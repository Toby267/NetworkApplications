...
class Traceroute(ICMPPing):
    ...
    def initialiseSocket(self, args):
    ...
    def sendIcmpProbesAndCollectResponses(self, ttl):
        ...
        for _ in range(3): 
            ...
            timeSent = self.sendIcmpProbe(ttl, dstPort, seq_num, numBytes, pkt_keys)

            sequenceNumReceived, timeRecvd, hopAddr = self.receiveAndParseICMPPacket()
            
            # 6. If the response matches the request, record the rtt and the hop address
            if sequenceNumReceived == seq_num:
    ...
    def sendIcmpProbe(self, ttl, dstPort, seq_num, numBytes, pkt_keys):
        # 1. Send one ICMP traceroute probe
        packetID = random.randint(1, 65535)
        timeSent = self.sendOnePing(self.dstAddress, packetID, seq_num, ttl, numBytes)

        # 2. Record a unique key (iCMP destination port) associated with the probe
        pkt_keys.append(dstPort)

        return timeSent

    def receiveAndParseICMPPacket(self):
        hopAddr = None
        icmpType = None
        ...
        # 4. Extract icmp from the reply assuming it reached the destination
        _, sequenceNumReceived, icmpType = self.parseICMPTracerouteResponse(trReplyPacket)

        # 5. Check if we reached the destination 
        if self.dstAddress == hopAddr and icmpType == 0:
            self.isDestinationReached = True

        return sequenceNumReceived, timeRecvd, hopAddr
    ...
    def sendUdpProbesAndCollectResponses(self, ttl):
        ...
        for _ in range(3): 
            ...
            timeSent = self.sendUdpProbe(ttl, dstPort, numBytes, pkt_keys)

            dstPortReceived, timeRecvd, hopAddr = self.receiveAndParseUDPPacket()
    ...
    def sendUdpProbe(self, ttl, dstPort, numBytes, pkt_keys):
        # 1. Send one UDP traceroute probe
    ...
    def receiveAndParseUDPPacket(self):
        hopAddr = None
        icmpType = None
        ...
        return dstPortReceived, timeRecvd, hopAddr
    ...
    def parseICMPTracerouteResponse(self, trReplyPacket):
        ...
        icmpType, _, _, p_id, sequenceNumReceived = struct.unpack('!BBHHH', trReplyPacket[ip_header_len:ip_header_len + 8])

        # destination is not reached so the response is in a different format
        # have to reparse the p_id and sequence number
        if icmpType == 11:
            origionalIndex = ip_header_len + 8
            _, _, _, p_id, sequenceNumReceived = struct.unpack('!BBHHH', trReplyPacket[ip_header_len + origionalIndex:ip_header_len + origionalIndex + 8])

        # return the packetID, sequence number and icmpType
        return p_id, sequenceNumReceived, icmpType
...
class MultiThreadedTraceRoute(Traceroute):
    ...
    def __init__(self, args):
        ...
        #0 initialise socket stuff from traceroute class
        self.initialiseSocket(args)
        ...
        self.seqNumsAndTimesent = dict()
        self.dstPortsAndTimesent = dict()
    ...
    def send_probes(self):
        ...
        while self.ttl <= MAX_TTL:
            ...
            # Notify the other thread and return if sending is complete
            ...
            for _ in range(3):  
                ...
                if args.protocol == "icmp":
                    timeSent = self.sendIcmpProbe(self.ttl, dstPort, seq_num, numBytes, self.pkt_keys)
                    self.seqNumsAndTimesent.update({seq_num: [timeSent, dstPort]})
                    
                elif args.protocol == "udp":
                    timeSent = self.sendUdpProbe(self.ttl, dstPort, numBytes, self.pkt_keys)
                    self.dstPortsAndTimesent.update({dstPort: timeSent})

            # Sleep for a short period between sending sets of probes
    ...
    def receive_responses(self):
        ...
        while not self.send_complete.is_set():
            ...
            if args.protocol == "icmp":
                ...
                if sequenceNumReceived in self.seqNumsAndTimesent:
                    timeSentAndDstPort = self.seqNumsAndTimesent[sequenceNumReceived]
                    dstPortReceived = timeSentAndDstPort[1]
                    timesent = timeSentAndDstPort[0]
                    
                    self.rtts[dstPortReceived] = timeRecvd - timesent
                    self.hop_addrs[dstPortReceived] = hopAddr

            elif args.protocol == "udp":
                dstPortReceived, timeRecvd, hopAddr = self.receiveAndParseUDPPacket()

                if dstPortReceived in self.dstPortsAndTimesent:
                    self.rtts[dstPortReceived] = timeRecvd - self.dstPortsAndTimesent[dstPortReceived]
                    self.hop_addrs[dstPortReceived] = hopAddr

            if packetsReceived % 3 == 0:
...
class Proxy(NetworkApplication):
    ...
    def __init__(self, args):
        ...
        # make the cache folder if it does not exist
